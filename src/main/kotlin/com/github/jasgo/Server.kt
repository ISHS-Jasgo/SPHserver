...
//            for (i in 0 until predictData.size) {
//                if (i % 2 == 0) {
//                    val place1 = Place.PLACE_NM.list[i]
//                    var place2 = ""
//                    if (i + 1 < predictData.size) {
//                        place2 = Place.PLACE_NM.list[i + 1]
//                    }
//                    val thread = Thread {
//                        simulate(place1, predictData)
//                        simulate(place2, predictData)
//                    }
//                    thread.start()
//                }
//            }
            parallelSimulation(predictData)
        }
    }, 0, 1000 * 60 * 5)
...
fun parallelSimulation(predictData: JSONObject) {
    val kernel = object : Kernel() {
        override fun run() {
            val gid = globalId
            val place = Place.PLACE_NM.list[gid]
            simulate(place, predictData)
        }
    }
    kernel.setExecutionModeWithoutFallback(Kernel.EXECUTION_MODE.GPU)
    kernel.execute(Place.PLACE_NM.list.size)
    kernel.dispose()
}
