...
//            println(positions.size)
        ...
        for (i in 0 until maxParticles) {
            ...
            for (j in 0 until neighbors.size) {
                ...
                var pressureForce =
                    (positions[i] - neighbors[j].second) / distance * (pressures[i] + pressures[j]) / (2 * densities[j]) * NORMALIZATION_PRESSURE_FORCE * (SMOOTHING_LENGTH.toDouble() - distance).pow(
                        2
                    )
